...
from btc_puzzle_lab.settings import NotifySettings
from btc_puzzle_lab.start import OperatorConfig, load_operator_config, operator_errors
...
def test_notify_skip_relay_does_not_post_back_to_hub():
    ...
    assert url is None or "discord.com" in str(post.call_args)
...
def test_hunt_config_does_not_need_dest():
    ...
    assert operator_errors(cfg, require_puzzle=True) == []


def test_config_relay_without_dest(capsys):
    _, pub = generate_relay_keypair()
    code = main(
        [
            "config",
            "--relay",
            "https://control.example:8787/hit",
            "--relay-seal-pubkey",
            pub,
            "--relay-token",
            _TOKEN,
    ...
    assert loaded.dest_addr == ""
    assert loaded.alert_configured is True
    assert operator_errors(loaded, require_puzzle=False) == []
...
def test_config_new_relay_token_prints_once(capsys):
    ...
    shown = [line.split(":", 1)[1].strip() for line in out.splitlines() if line.startswith("relay token :")]
    assert shown and len(shown[0]) >= 16
    loaded = load_operator_config()
    assert loaded.relay_token == shown[0]
    assert main(["config"]) == 0
    again = capsys.readouterr().out
    assert shown[0] not in again
    assert "relay token: set" in again
